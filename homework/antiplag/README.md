...
Для документирования API используется **Swagger / OpenAPI** на базе библиотеки `springdoc-openapi-starter-webmvc-ui`.

Swagger подключен во всех трёх сервисах:

* `gateway-service` — `http://localhost:8080/swagger-ui/index.html`
* `storage-service` — `http://localhost:8081/swagger-ui/index.html`
* `analysis-service` — `http://localhost:8082/swagger-ui/index.html`

Через Swagger UI можно:

* увидеть все доступные эндпоинты каждого микросервиса (внешние и `internal`),
* посмотреть форматы запросов и ответов,
* интерактивно вызывать ручки прямо из браузера.

OpenAPI-спецификация также доступна в машиночитаемом виде по стандартному пути `/v3/api-docs` в каждом сервисе.

Таким образом, требование
**«Реализация коллекции Postman / Swagger, демонстрирующей функциональность микросервисов и охватывающей все API»**
выполнено за счёт Swagger-документации для каждого микросервиса.
